...
namespace Payroll
{
    public class ReportEngine
    {
        ...
        static string GetReportData(ReportBuilder reportBuilder)
        {
            ...
            byte[] imgBinary = File.ReadAllBytes(HttpContext.Current.Server.MapPath("assets") + "\\images\\logo.png");
  ...
  <DataSets>"
     ...
     " + GetDataSetFields(tables[i].ReportDataColumns) + @" 
        ...
        private static string GetDataSetFields(ReportColumns[] reportColumns)
        {
            ...
            {
                strFields += @"<Field Name=""" + reportColumns[i].ColumnCell.Name.Replace(" ", "") + @"""> 
        ...
        static string GenerateTableRow(ReportBuilder reportBuilder, ReportTable table)
        {
            ...
            for (int i = 0; i < columns.Length; i++)
            {
                ...
                strTableRow += @"<TablixCell> 
                  <CellContents> 
                   " + GenerateTextBox("txtCell_" + table.ReportName + "_", ColumnCell.Name.Replace(" ", ""), "", true, padding) + @" 
                  </CellContents> 
                </TablixCell>";
            }
        ...
        static string GenerateTableHeaderRow(ReportBuilder reportBuilder, ReportTable table)
        {
            ...
            for (int i = 0; i < columns.Length; i++)
            {
                Headertextvalue = columns[i].HeaderText;
                ColumnCell = columns[i].ColumnCell;
                padding = columns[i].HeaderColumnPadding;
                strTableRow += @"<TablixCell> 
                  <CellContents> 
                  
                   " + GenerateHeaderTableTextBox("txtHeader_" + table.ReportName + "_", ColumnCell.Name.Replace(" ", ""), Headertextvalue == null || Headertextvalue.Trim() == "" ? ColumnCell.Name.Replace(" ", "") : Headertextvalue, false, padding) + @" 
        ...
        {
            string strTextBox = "";
            strTextBox = @" <Textbox Name=""" + strControlIDPrefix + strName.Replace(" ", "") + @"""> 
            ...
            strTextBox += @"<Style /> 
                      ...
                      <rd:DefaultName>" + strControlIDPrefix + strName.Replace(" ", "") + @"</rd:DefaultName> 
                      <Style> 
                        <Border> 
